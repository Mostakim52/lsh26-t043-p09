 ...
 *
 * Ported from frontend/src/engine/rules.ts - only readNumberEnv differs (process.env
 * instead of Vite's import.meta.env). Every formula and constant below must stay
 * byte-identical to the frontend's copy; that file is the spec.
 */

/** An item falling due within this many days counts as "due soon". */
export const DUE_SOON_DAYS = readNumberEnv('DUE_SOON_DAYS', 30);

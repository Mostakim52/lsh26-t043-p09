...
import { buildCallList, summariseFleet } from './callList';
import { completeService, historyForVehicle, resetRule } from './complete';
import { addMonths, addDays, daysBetween, relativeDays } from './dates';
import { DUE_SOON_DAYS } from './rules';
import { computeSchedule, computeVehicleStatus, projectOdometer, statusFor } from './schedule';
import type { Fleet, ServiceItem, Vehicle } from './types';

const fleet = JSON.parse(
  readFileSync(resolve(__dirname, '../../public/data/fleet.json'), 'utf8'),
) as Fleet;
